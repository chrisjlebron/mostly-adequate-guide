...
var _average = function(xs) { return s.reduce(s.add, 0, xs) / xs.length; }; // <- leave be

// var averageDollarValue = function(cars) {
//   var dollar_values = s.map(function(c) { return c.dollar_value; }, cars);
//   return _average(dollar_values);
// };

// 1.
// var averageDollarValue = _.compose(_average, s.map(c => c.dollar_value));

// 2.
var averageDollarValue = _.compose(_average, s.map(_.prop('dollar_value')));
...
var _underscore = s.replace(/\W+/g, '_'); //<-- leave this alone and use to sanitize


// var sanitizeNames = undefined;

// 1.
// var sanitizeNames = _.compose(s.map(c => _underscore(s.toLowerCase(c.name))));

// 2.
var sanitizeNames = s.map(
  _.compose(
    _underscore,
    s.toLowerCase,
    _.prop('name')
...
const availablePrices = _.compose(
  s.join(', '),
  s.map(x => accounting.formatMoney(x.dollar_value)),
...
//   var sorted = _.sortBy(car => car.horsepower, cars);
//   var fastest = _.last(sorted);
//   return fastest.name + ' is the fastest';
// };

// 2.
const append = _.flip(s.add);
